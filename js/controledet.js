...
const botaoAdicionarTurma = document.querySelector(".botao-adicionar-turma");
...
function acessarTurma(botao) {
    ...
    const cardTurma = botao.closest(".turma");

    const nomeTurma = cardTurma
        .querySelector(".cabecalho-turma h2")
        .textContent
        .trim();

    alert("Você acessou a " + nomeTurma + ".");
}


// Botões das turmas que já existem
const botoesAcessarTurma =
    document.querySelectorAll(".botao-acessar-turma");
...
botaoAdicionarTurma.addEventListener("click", function() {

    numeroTurma++;

    const numeroFormatado =
        numeroTurma.toString().padStart(2, "0");
    ...
    novaTurma.innerHTML = `
        ...
        <div class="cabecalho-turma">
            ...
            <h2>TURMA ${numeroFormatado}</h2>

            <span class="icone-turma">●</span>
    ...
    const botaoAcessarNovaTurma =
        novaTurma.querySelector(".botao-acessar-turma");

    botaoAcessarNovaTurma.addEventListener(
        "click",
        function() {
            acessarTurma(botaoAcessarNovaTurma);
        }
    );
...
perguntas.forEach((pergunta) => {
    ...
    const botaoPergunta =
        pergunta.querySelector(".botao-pergunta");

    const respostaPergunta =
        pergunta.querySelector(".resposta-pergunta");
    ...
    botaoPergunta.addEventListener("click", function() {
        ...
        perguntas.forEach((outraPergunta) => {
            ...
            const outroBotao =
                outraPergunta.querySelector(".botao-pergunta");
